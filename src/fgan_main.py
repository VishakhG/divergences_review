...
from utils import Generator, pdf_normal
...
for epoch_i in range(n_epochs):
    ...
    for _, batch_real in enumerate(dataloader_train):
        ...
        d_loss = discrimator_loss(d_real, d_fake, divergence_choice=divergence_choice)
...
#plt.savefig("/Users/Vishakh/Desktop/fgan.png")
...
# plt.suptitle('F-gan samples with divergence '+(args.divergence_type.lower().replace("_", " ")), fontsize=16)

# plt.savefig("../plots/f_gan_samples_{0}.png".format(args.divergence_type))
...
plt.savefig('../plots/F-gan samples with divergence ' +(args.divergence_type.lower().replace("_", "")))
