...
class GMM():
    ...
    def normal(self, X, m, C):
        Z = X - m
        return tt.exp(-tt.sum(Z * tt.dot(tl.matrix_inverse(C), Z), 0) / 2. - tt.log(tl.det(C)) / 2. - m.size / 2. * np.log(2. * np.pi))
    ...
    def evaluate_density(self, data):
        ...
        def log_p(X):
            if isinstance(X, tt.TensorVariable):
                return tt.log(tt.sum([self.mix_weights[i] * self.normal(X, self.means[:, [i]], self.covs[i]) for i in range(len(self.mix_weights))], 0))
            else:
                if log_p.f is None:
                    Y = tt.dmatrix('Y')
    ...
    def sample(self, n_samples):
        ...
        data = np.hstack(npr.randn(self.dim, M[i]) * np.sqrt(self.vars[i]) + self.means[:, [i]] for i in range((self.n_gaussians)))
...
# gmm = GMM(2, 2, 1)
# data = gmm.sample(100)
# likelihood = - gmm.evaluate_density(data)
#
# plt.scatter(data[:,0], data[:, 1])
#
# means = gmm.means.T
# plt.scatter(means[:,0], means[:,1], c='red')
# # sns.regplot(means[:,0], means[:,1], scatter=True, fit_reg=False, color='')
# #
# #
# #
# # sns.set_style("white")
...
# # sns.kdeplot(data[:,0], data[:,1], cmap="Blues", shade=True, bw=.15)
#
# plt.show()
